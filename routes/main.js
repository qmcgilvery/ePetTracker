...
module.exports = function (app) {
  ...
  app.get("/update", function (req, res) {
    ...
    // for new_status page -- to show walk info (COLLIN)
    app.get("/about", function (req, res) {
      // query database to get all the walk_1 table
      let sqlquery_2 = "SELECT * FROM walk_1";
      // execute sql query
      db.query(sqlquery_2, (err, result) => {
        if (err) {
          res.redirect("/");
        }
  ...
  app.post("/add", function (req, res) {
      ...
      if (req.body.add_walk) {
        console.log(req.body)
        let sqlquery2 = "INSERT INTO walk_1 (walk_name, walk_distance, walk_datetime, pet_id) VALUES (?, ?, ?, ?)";
